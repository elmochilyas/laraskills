...
- **Wildcard as a Structural Mirror** — The `*` wildcard mirrors the array structure, applying rules uniformly across all elements without explicit iteration in application code.
- **Array Validation as a Schema Language** — Array validation rules describe the schema of nested API payloads declaratively, serving as a machine-readable data contract.
- **Dot Notation as a Navigation System** — Dot notation navigates nested structures, providing a consistent path language from validation errors to input location.
- **Array Constraints as Resource Bounds** — Min/max constraints on arrays bound the resource consumption of validation, preventing denial-of-service through massive nested payloads.
