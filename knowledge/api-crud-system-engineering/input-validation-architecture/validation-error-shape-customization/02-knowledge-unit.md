...
- **Error Shape as API Contract** — The validation error format is part of the API contract; it must be consistent, documented, and versioned alongside the rest of the API.
- **failedValidation() as a Centralized Exception Handler** — Overriding `failedValidation()` in a base `ApiRequest` provides a single point of error formatting for all endpoints, ensuring consistency.
- **Error Structure as Developer Experience** — A well-structured error response reduces client-side parsing complexity and speeds up debugging for API consumers.
- **Consistency as a Trust Signal** — Consistent error shapes across all endpoints build client trust and reduce integration time for third-party consumers.
