...
- **Directory Structure as API Map** — The filesystem mirrors the API surface; each resource subdirectory corresponds to an API endpoint group, making navigation intuitive.
- **Per-Action Files as Single-Responsibility Units** — Each action (Store, Update, Index) gets its own file, enforcing single responsibility and explicit naming.
- **Base Requests as Shared Contracts** — Base request classes capture shared rules and error handling across related endpoints, reducing duplication.
- **Organization as Developer Navigation** — A well-organized request directory lets developers find validation logic by knowing only the endpoint URL.
