...
- **Conditional Validation as a Decision Tree** — Each condition creates a branch in the validation logic; the input shape determines which branch is taken and which rules apply.
- **sometimes as an Optional Gate** — The `sometimes` rule is an optional gate: it validates only when the field is present, otherwise it remains open and skips all subsequent rules.
- **Rule::when() as a Ternary Operator** — `Rule::when()` acts as a ternary operator for rule sets: condition ? applyRulesA : applyRulesB.
- **after() as a Cross-Field Arbiter** — The `after()` hook resolves conflicts between interdependent fields that cannot be validated in isolation.
