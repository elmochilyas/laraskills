...
- **toDto() as an Explicit Converter** — Unlike `payload()` which is request-bound, `toDto()` is an explicit conversion that can live outside the request, decoupling DTO construction from HTTP concerns.
- **Mapper as a Version Bridge** — Standalone mappers bridge different API versions by converting between request shapes and DTO structures independently.
- **toDto() as a Transformer** — Transforms validated data into domain objects, applying additional logic like slug generation, enumeration casting, or default injection.
- **Separation of Concerns in Conversion** — Keeping conversion logic separate from the request class allows the mapping to evolve independently from validation rules.
