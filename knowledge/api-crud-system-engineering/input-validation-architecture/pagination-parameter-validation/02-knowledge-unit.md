...
- **Pagination as a Performance Control** — Pagination parameters are performance controls, not just UI concerns; unvalidated pagination is a resource exhaustion vulnerability.
- **per_page as a Resource Budget** — The `per_page` parameter is a budget that bounds how many resources a single request can consume from the database and memory.
- **Defaults as Safety Nets** — Sensible defaults for `page` and `per_page` protect the system when clients omit these parameters, preventing unbounded queries.
- **Cursor as an Opaque Token** — Cursor-based pagination treats the cursor as an opaque token whose format must be validated to prevent injection or manipulation attacks.
