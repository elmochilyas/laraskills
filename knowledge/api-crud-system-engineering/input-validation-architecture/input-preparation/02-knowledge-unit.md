...
- **prepareForValidation() as a Normalization Layer** — Prepares raw HTTP input into a normalized form before validation, separating sanitization concerns from rule evaluation.
- **Sanitization as a Pre-Processor** — Input preparation acts as a pre-processor pipeline that cleans, coerces, and defaults input before the validator sees it.
- **Defaults as Silent Contracts** — Default values inject optionality into the API contract; clients can omit fields and the system applies sensible fallbacks.
- **Merge as a Transformation Buffer** — The `merge()` operation buffers transformations in a single pass, allowing multiple preparation steps before the validator runs.
