...
- **Slug:** local-llm-setup---configuration
- **Version:** 1.0.0
- **Maturity:** Stable
- **Status:** Published
...
The internal mechanics of Local LLM Setup & Configuration follow established patterns within the Local LLMs domain. The implementation leverages the Laravel AI SDK conventions and ecosystem best practices.
...
- For production, use **vLLM** or **TGI** (Text Generation Inference) â€” they support continuous batching and higher throughput than Ollama.
- Separate **inference serving** from **application serving** â€” the Laravel app talks to the inference server over HTTP.
...
- **Authentication:** If the inference server must be network-accessible, add API key authentication (Ollama doesn't support auth natively â€” use a reverse proxy).
