...
status: "standardized"
file-path: "research/workspaces/ai-intelligence-systems/cost-management-observability/ku-03/04-standardized-knowledge.md"
---
...
- **Service Level Indicators (SLIs):** Measurable aspects of service performance — latency (p50/p95/p99), error rate, throughput, cost per request, cache hit rate.
...
- **Distributed Tracing:** End-to-end request tracing from user → application → gateway → LLM provider → back.
...
- Production AI systems of any scale — observability is required for reliability.
- Systems with multiple providers — comparative provider metrics guide routing decisions.
- Applications with SLO commitments — monitoring is necessary to measure and report SLO compliance.
- Compliance-managed systems requiring audit trails.

## When NOT To Use

- Local development (basic logging suffices).
- Short-lived prototypes that are not deployed.
...
- Instrument all services with **OpenTelemetry** for traces, metrics, and logs — vendor-neutral and portable.
...
- Building dashboards before defining SLOs — dashboards without targets are decoration.
- Alert fatigue: too many alerts that nobody responds to. Start with 3-5 critical alerts and expand cautiously.
- Not monitoring cost — the most expensive AI incident is not a technical failure but a budget overrun discovered on the invoice.
- Missing provider-specific alerts — a provider outage with no alert means users discover the outage before the team does.
- Not documenting runbooks — when an alert fires at 3 AM, the on-call engineer needs clear instructions.
...
```
# Latency
ai_request_duration_seconds{provider="openai",model="gpt-4o"} — p50, p95, p99

# Errors
ai_request_errors_total{provider="openai",error_type="timeout"} — rate per minute

# Cost
ai_cost_usd_total{provider="openai",model="gpt-4o"} — per hour/day

# Cache
ai_cache_hit_ratio{provider="openai"} — per provider

# Tokens
ai_prompt_tokens_total{provider="openai"} — per minute
...
- When asked to set up observability, first ask: what are the SLOs? What is the expected traffic volume? What is the on-call rotation?
- For observability issues, check: metric naming consistency, log format, and correlation ID propagation.
- Prefer reading the SLO configuration before the dashboard definitions — SLOs define what's important.
- When generating alerting rules, always include a runbook reference and suggested response actions.
