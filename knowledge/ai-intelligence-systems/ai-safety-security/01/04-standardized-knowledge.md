...
status: "standardized"
file-path: "research/workspaces/ai-intelligence-systems/ai-safety-security/ku-01/04-standardized-knowledge.md"
---
...
- Applications with multi-tenant user input — one user should not affect others' prompts.
...
- Situations where the cost of prevention outweighs the risk (e.g., internal tools with trusted users only).
...
- **Validate user input** for known injection patterns (but don't rely solely on pattern matching — it's easily bypassed).
...
- Implement injection prevention at **multiple layers**: input validation → prompt construction → runtime detection → output validation.
...
- **Monitor for injection attempts** — they indicate active attacks, not just bugs. Log and alert on detected injections.
- **Consider the supply chain:** Third-party plugins, models, and tools may introduce injection vectors.

## Common Mistakes

- Concatenating user input directly into the system prompt — the most common and dangerous mistake.
- Relying solely on the LLM's alignment training to reject injection attempts.
- Not treating retrieved documents as untrusted — indirect injection attacks are on the rise.
- Using only input filtering (blacklist) without structural defenses (role separation, delimiters).
- Not testing injection resistance — assuming it works without dedicated security testing.
...
- When asked to fix prompt injection vulnerabilities, first check: are user inputs ever concatenated into system prompts? Are retrieved documents treated as untrusted?
- For injection incident analysis, request: the full prompt (with system message), user input, and LLM response.
- Prefer reading the prompt construction code before the sanitizer — the architecture reveals the attack surface.
- When generating code that handles user input → LLM, always use role separation and delimiter wrapping by default.
