...
status: "standardized"
file-path: "research/workspaces/ai-intelligence-systems/ai-safety-security/ku-04/04-standardized-knowledge.md"
---
...
- Fully on-premise deployments where data never leaves your infrastructure (still apply PII protection best practices but with lower risk).
...
- **Provide data deletion APIs** — users must be able to request deletion of their data (conversations, embeddings, profiles).
...
- For RAG systems, store embeddings in an **isolated vector index** per tenant or per user — never mix PII across tenants.
- The PII redaction layer should be **provider-agnostic** — redact before the provider-specific transformation.
...
- Sending PII to providers despite zero-retention claims — "zero retention" is not "zero exposure".
...
- When asked to implement PII protection, first identify: what data flows to external LLM providers, what is the regulatory context, and what is stored long-term.
- For PII-related issues, check: redaction patterns completeness, de-redaction mapping, and log retention policies.
- Prefer reading the privacy policy configuration before the redaction middleware implementation.
- When generating streaming code with PII protection, ensure per-chunk processing handles partial matches correctly.
