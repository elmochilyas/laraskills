---
id: ku-03
title: "Secure Secrets & Configuration Management"
subdomain: "ai-safety-security"
ku-type: "security"
date-created: "2026-06-02"
domain-maturity: "mature"
status: "standardized"
file-path: "research/workspaces/ai-intelligence-systems/ai-safety-security/ku-03/04-standardized-knowledge.md"
---
...
- Multi-environment deployments (dev/staging/prod) requiring different credentials per environment.
...
- Local development prototyping (environment variables are acceptable for now, but plan to move to secrets manager).
- Single-developer, single-deployment hobby projects (the overhead of a secrets manager may exceed the risk).
...
- **Implement emergency revocation** — the ability to revoke a compromised secret within seconds across all services.
...
- **Secret sprawl:** Regularly audit secrets in the vault — remove unused or expired secrets.
...
- Not rotating secrets — a year-old secret is almost certainly compromised.
- Logging secrets in application logs or error messages.
- Hardcoding fallback values in config files when the vault is unreachable — creates a leak vector.

## Anti-Patterns

- **Config Files as Secrets:** Encrypting config files and committing them to git. Prefer a secrets manager with dynamic secrets.
- **Hardcoded Defaults:** `config('ai.openai_key') ?? 'sk-...'` — never put real keys as defaults.
...
- When asked to set up secrets management, first identify all secrets in the system and classify them by sensitivity.
- For secret-related incidents, check: git history (committed secrets), logs, and error messages for exposure.
- Never include real secrets in any generated code, configuration files, or documentation.
- Prefer reading the secrets manager integration before the application config to understand the security model.
