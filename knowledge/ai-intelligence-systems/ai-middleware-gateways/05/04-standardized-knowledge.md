...
status: "standardized"
file-path: "research/workspaces/ai-intelligence-systems/ai-middleware-gateway/ku-05/04-standardized-knowledge.md"
---
...
- **Cost Tracking:** Per-request cost (computed from token counts × provider pricing), aggregated by application, team, or feature.
- **Model Behavior Metrics:** Response length, tool call frequency, refusal rate, streaming chunk rate.
- **Distributed Tracing:** End-to-end trace from application → gateway → provider → gateway → application.
...
- Implement observability as **middleware** in the gateway pipeline — every request passes through the observability middleware.
...
- Cost computation involves a lookup and multiplication — cache the pricing table and compute in <1ms.
...
- **Not tracking cost** — without per-request cost data, it's impossible to optimize spend or bill internally.
- **Sampling error logs** — errors should always be logged at full fidelity, never sampled.
- **Missing latency breakdown** — knowing total latency is insufficient. Break down into: gateway overhead, provider latency, and transform time.
- **Ignoring cache metrics** — cache hit rate, miss rate, and eviction rate are critical for understanding efficiency.
- **Not setting up alerts** — discovering a provider outage through user complaints is unacceptable.
...
- When asked to set up observability, first identify the key stakeholders (who needs to see what) and the data retention requirements.
- For monitoring-related bugs, check: metric naming consistency, log format, and whether the observability middleware is in the correct pipeline position.
- Prefer reading the observability middleware configuration before individual metric implementations.
- When generating dashboards, start with the "four golden signals": latency, traffic, errors, and saturation (cost).
...
- [ ] Log retention policy is defined and enforced (monthly/yearly purging).
