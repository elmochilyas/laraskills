...
status: "standardized"
file-path: "research/workspaces/ai-intelligence-systems/retrieval-augmented-generation/ku-05/04-standardized-knowledge.md"
---
...
- Initial RAG system development — establishing baseline retrieval quality.
- After any change to the retrieval pipeline (chunking, embedding model, search algorithm, reranking).
- Periodic monitoring — retrieval quality can degrade as the document corpus grows or changes.
- Comparing embedding models — which model works best for your specific corpus and query types.

## When NOT To Use

- Prototypes where any retrieval is better than none.
- Systems with manual document selection (human-curated context).
...
- Running a full evaluation suite (100 queries × multiple K values) takes 1-10 minutes (embedding + search).
- Evaluation queries should run against a **frozen snapshot** of the vector index to ensure reproducible results.
- Computed metrics can be cached — if the pipeline hasn't changed, neither have the metrics.
...
- Only measuring Precision@K without Recall@K — you find only relevant documents but miss many relevant ones.
- Using a test collection that doesn't represent real user queries — lab metrics don't match production performance.
- Not segmenting by query type — optimizing for the average may hurt specific query categories.
- Evaluating retrieval without evaluating end-to-end RAG quality — good retrieval + bad generation = bad answers.
- Over-optimizing for a test collection (overfitting) — the pipeline may not generalize to unseen queries.
- Not tracking quality over time — degradation goes unnoticed until users complain.
...
- **No Baseline:** Implementing retrieval improvements without a baseline measurement — you can't measure improvement.
...
- When asked to improve RAG quality, start by evaluating current retrieval quality with a test collection.
- For retrieval quality issues, check: chunking strategy, embedding model, hybrid search configuration, and reranking.
- Prefer reading the test collection and recent evaluation results before making recommendations.
- When generating retrieval evaluation code, include: test collection management, metric computation, and CI integration.

## Verification

- [ ] Test collection of 100-500 query-relevant document pairs exists.
- [ ] Precision@K and Recall@K are measured (not just one).
- [ ] Evaluation is segmented by query type (factual, comparative, procedural).
- [ ] Retrieval quality evaluation is automated in CI.
- [ ] Quality metrics are tracked over time (weekly charts).
- [ ] Test collection is version-controlled and reviewed for quality.
