...
status: "standardized"
file-path: "research/workspaces/ai-intelligence-systems/prompt-engineering-systems/ku-02/04-standardized-knowledge.md"
---
...
- **Capability Declaration:** What the agent can do — which tools it has access to, what data sources it can query.
...
- **Context Window Strategy:** How the agent should manage its context window — when to summarize, when to ask for clarification.
...
- When the model's default behavior (without system prompt) already produces the desired output.
...
- Implement **environment-specific system prompts** — development may have relaxed constraints, production enforces strict rules.
...
- A verbose system prompt adds tokens to every request. Optimize for the common case — don't include rarely-used instructions.
...
- Writing system prompts that are too long (3000+ tokens) — the model ignores or forgets instructions.
- Including contradictory instructions (e.g., "Be concise" and "Provide a detailed analysis").
- Not testing system prompt changes against edge cases — a small change can break behavior.
- Assuming the model will follow all instructions perfectly — test, don't trust.
- Using the same system prompt for different tasks — a chat agent and a data extraction agent need different prompts.
...
- Process refunds within the 30-day policy
- Create support tickets for complex issues

## Constraints
- ONLY use information from the provided knowledge base
- NEVER make up pricing or policy details
- If you don't know the answer, say "I'll transfer you to a specialist"
...
- Do not share customer PII with the user
PROMPT;
    }
}
```
...
class SystemPromptBuilder {
    public function build(string $role, array $overrides = []): string {
        $sections = [
            'persona' => $this->getPersonaSection($role),
            'capabilities' => $this->getCapabilitiesSection($role),
            'constraints' => $this->getConstraintsSection($role),
            'output' => $this->getOutputFormatSection($role),
            'safety' => $this->getSafetySection($role),
        ];

        $sections = array_merge($sections, $overrides);
...
- When asked to design a system prompt, first understand: the agent's role, available tools, target users, and safety requirements.
- For system prompt bugs, check: instruction ordering, contradictions, and whether instructions are specific enough.
- Prefer reading the system prompt builder before individual prompts — the composition strategy reveals the design.
- When generating system prompt code, include: modular sections, versioning, and environment-specific overrides.
