...
## Research Notes
Database branching is a paradigm shift in development database management. The cost model (zero for idle branches, pay only for compute when used) makes it feasible to give every developer multiple databases. This is especially powerful for Laravel apps with complex database schemas where testing requires realistic data. Aurora Global Database provides a limited form of branching via clone, but at significantly higher cost.
