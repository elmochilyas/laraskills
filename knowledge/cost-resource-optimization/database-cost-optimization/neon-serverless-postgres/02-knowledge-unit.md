...
- **Database branching**: Instant copy-on-write; zero-cost for CI/CD, dev, testing environments
- **Laravel Cloud default**: Neon is the default database option for new Laravel Cloud projects

## Mental Models
- **Neon as Git for databases**: Branching creates instant, zero-cost database copies for each PR/environment
- **Scale-to-zero as hibernation**: Database sleeps when not used; you pay only for storage
...
## Research Notes
Neon's sub-1s cold start and branching model make it uniquely suited for Laravel development workflows. The 100 compute-hour free tier is generous enough for most staging environments. For production at scale, compute costs are comparable to Aurora Serverless v2. The key advantage is branching (CI/CD integration) and scale-to-zero (cost efficiency for variable workloads). Neon uses a disaggregated storage architecture (pageservers) that separates compute from storage, enabling instant branching and rapid cold starts.
