...
## Best Practices
- **Use RoadRunner for Octane over Swoole**: Simpler Docker builds, no PHP extension dependency (WHY: `rr` binary is static Go; `php-rr` in Dockerfile is a single ADD command vs Swoole requiring pecl install, compilation, extension loading)
- **Configure worker pool size**: Set `workers` and `max_jobs` in .rr.yaml based on server resources (WHY: workers = CPU cores for CPU-bound; max_jobs = 500 resets workers periodically to prevent memory leaks)
- **Serve static files from RoadRunner**: Enable `static` plugin to serve assets directly without Nginx (WHY: reduces deployment complexity; `rr serve` is the only process needed; no reverse proxy required)
- **Monitor RoadRunner metrics**: Enable `rpc` plugin for Prometheus metrics (workers, jobs, memory) (WHY: visibility into worker health prevents silent failures; metrics reveal memory leaks early)
- **Use .env per environment**: RoadRunner reads `.env` via Octane; configure per-environment .rr.yaml (WHY: development needs fewer workers, different file paths; production needs higher limits)
...
## Common Mistakes
1. **Not configuring max_jobs**: Workers never restart; memory grows unbounded (Cause: assuming Octane workers are immortal; Consequence: memory leaks compound; workers crash after hours/days; Better: set max_jobs: 500 to restart workers after 500 requests)
2. **Over-allocating workers**: Setting workers > 2x CPU cores for I/O-bound app without monitoring context switching (Cause: "more workers = more throughput"; Consequence: CPU thrashing, throughput drops; Better: start with workers = CPU cores, benchmark, increase to 1.5x for I/O-heavy)
3. **No health check for RoadRunner**: Assuming RR never fails (Cause: Nginx-based thinking; Consequence: RR crash causes total downtime; Better: configure ALB health check on `/health` endpoint, use `supervisord` auto-restart)
