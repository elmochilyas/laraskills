...
## Best Practices
- **Set pool size to 2-3x database vCPUs**: For 4-vCPU database, default_pool_size = 8-12 (WHY: database processes connections efficiently up to ~2x vCPU count; more active connections cause context switching overhead; fewer connections underutilize database throughput)
- **Monitor connection utilization**: Track active_connections / max_connections; alarm at 80% (WHY: prevents connection exhaustion errors; 80% gives buffer for traffic spikes without wasting idle connection slots)
- **Right-size pool based on connection duration**: Short transactions (<100ms) tolerate higher multiplexing; long transactions (reports, exports) need more pool connections (WHY: long transactions hold connections longer, requiring larger pools or lower multiplexing)
- **Use separate pools for read vs write**: Direct write connections to primary pool, read connections to reader pool (WHY: prevents read-heavy queries from blocking write capacity; each pool sized independently)
- **Avoid connection leaks**: Ensure database connections are released in `finally` blocks or PHP-FPM max_requests resets them (WHY: leaked connections reduce pool capacity; PHP-FPM's max_requests automatically kills leaked connections, but pooler doesn't know they're leaked)
...
## Common Mistakes
1. **Over-sized pool**: Setting PgBouncer default_pool_size = 100 for 2-vCPU database (Cause: "more connections = more capacity"; Consequence: 100 active connections overwhelm 2-vCPU database; context switching reduces throughput 30%+; Better: 6-8 connections for 2-vCPU DB)
2. **No pool size monitoring**: Setting pool size to 25 and never checking utilization (Cause: set-and-forget configuration; Consequence: pool at 100% capacity, requests queueing, no alert; Better: monitor active_connections/max_connections with CloudWatch alarm)
3. **Using RDS Proxy without IAM auth**: Storing database password in connection strings (Cause: default behavior; Consequence: password rotation requires deploy; Better: enable IAM auth, Laravel handles credential generation automatically)
