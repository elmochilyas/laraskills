# KU-02-PERSISTENT-CONNECTIONS: Persistent Connections
...
## Patterns
- Enable persistent connections in PHP-FPM
- Implement connection health checks
- Use Octane's connection management
- Set `wait_timeout` higher than idle time
...
## Common Mistakes
- **Persistent connections without health checks**: Connection silently drops, next request fails (Cause: assuming TCP connections never break; Consequence: "MySQL has gone away" errors affecting 0.1-1% of requests; Better: implement SELECT 1 health check every 30 seconds)
- **Persistent connections + PgBouncer transaction pooling**: PgBouncer holds transaction-scoped connections; persistent connections at PHP level conflict (Cause: two layers of pooling with incompatible assumptions; Consequence: connection count blowup, unexpected disconnects; Better: disable PHP persistent connections when using PgBouncer)
- **No max_requests with persistence**: Workers never restart, connections accumulate memory, grow stale indefinitely (Cause: "persistent connection forever" assumption; Consequence: connections eventually fail or leak memory; Better: set max_requests = 500 to refresh workers and connections periodically)
- **Octane without connection refresh**: Long-running Octane workers holding same connections for days (Cause: workers process millions of requests; Consequence: connections become stale, network timeouts break them; Better: use Octane::tick() to refresh connections every N requests)
