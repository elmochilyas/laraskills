...
## Best Practices
- **Set TTL on every key**: Even if data rarely changes, set 24h TTL with cache warming (WHY: prevents indefinite memory accumulation; Redis maxmemory eviction is less predictable)
- **Use Laravel cache tags**: `Cache::tags(['users', 'active'])->put($key, $value, $ttl)` enables atomic flush (WHY: `Cache::tags('users')->flush()` invalidates all user cache in one operation without flushing entire Redis DB)
- **Stagger TTL by +/-10%**: For popular keys, add random jitter to TTL (WHY: prevents all keys expiring simultaneously, causing origin load spike and cache stampede)
- **Prefix queues by environment**: `queues:production:emails` vs `queues:staging:emails` (WHY: prevents cross-environment queue processing disasters)
...
## Common Mistakes
1. **No TTL on cache entries**: Keys accumulate until maxmemory, then random eviction starts (Cause: developer assumes data fits in memory forever; Consequence: unpredictable cache behavior; Better: always set explicit TTL)
2. **Flushing entire cache in deployment**: Using `php artisan cache:clear` in production (Cause: convenience script; Consequence: all cache entries expire simultaneously, origin overload for 5-30 minutes; Better: use tag-based flush or versioned prefixes)
3. **Short TTL on all data**: Setting 5-minute TTL on everything "to keep data fresh" (Cause: misunderstanding TTL purpose; Consequence: 95% cache miss rate, no benefit from cache; Better: set TTL based on data staleness tolerance)
