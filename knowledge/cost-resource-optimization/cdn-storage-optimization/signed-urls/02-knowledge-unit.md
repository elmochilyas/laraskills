...
## Patterns
- Use CloudFront origin access control (OAC) + signed URLs
- Short expiration for APIs
- Cache signed URLs at CloudFront
- Use signed cookies for multiple files
...
- **Overly long expiration**: Setting signed URL expiration to 24 hours for download links (Cause: convenience for user; Consequence: URL can be shared widely; Better: 1-5 minute expiration; user must visit site to get new URL)
- **Storing private key in repository**: Committing .pem file to Git (Cause: convenience during development; Consequence: private key compromised if repo is breached; Better: store in environment variables or Secrets Manager, never in repo)
