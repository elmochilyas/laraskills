...
## Executive Summary
CloudFront Origin Shield adds a regional caching layer between edge locations and your origin, reducing origin requests by 70-90% for global audiences. Instead of each edge POP requesting content from the origin independently, all edges route through a single shield POP. This reduces origin load, improves cache hit ratio, and reduces origin egress costs. Shield is free for CloudFront Ã¢â‚¬â€ no additional charge beyond standard CloudFront pricing.

## Core Concepts
- **How it works**: Regional intermediate cache between edge POPs and origin
- **Origin load reduction**: 70-90% fewer origin requests
- **Cost impact**: Reduces origin data transfer, S3 request costs, compute
- **Location**: Choose shield region close to origin
- **Pricing**: Included in CloudFront pricing (no additional fee)
...
## Research Notes
Origin Shield is most beneficial for apps with global audiences and cacheable content. For Laravel apps serving most traffic from one region, the benefit is minimal. For apps with users in 10+ countries, shield significantly reduces origin load. Configure shield region to be the same as your origin server region for lowest latency.
