...
- **Source**: AWS Documentation, Industry Research
...
## Mental Models
- **Predictive as weather forecast**: You know rain is coming (traffic) and prepare beforehand vs waiting to get wet and then buying an umbrella (reactive)
- **Proactive vs reactive**: Predictive scaling prepares for demand; step scaling chases it
...
## Research Notes
Predictive scaling requires 14 days of traffic history to train the model. Best results are achieved with daily/weekly cyclical patterns. For Laravel apps with predictable traffic (e.g., B2B apps that peak during business hours, e-commerce with weekend peaks), predictive scaling reduces overprovisioning waste. Combine with step scaling for real-time adjustment (reactive safety net). Predictive scaling is less effective for apps with random or non-cyclical traffic patterns.
