...
- **Owned Code**: Breeze gave you the keys to the car. You could customize anything because the code was in your application directory.
- **Security Maintenance Required**: Because controllers were in your app, security fixes from Laravel (e.g., rate limiting on login) did not automatically apply. You had to manually update published code.

---

# Migration Path

- Replace Breeze controllers with Fortify configuration (headless backend). Remove published controllers from `app/Http/Controllers/Auth/`. Configure Fortify features.
- For existing projects: consider keeping Breeze until the next major upgrade. For new projects: use Laravel 13 Starter Kits.
- The migration guide from Breeze to Fortify + Starter Kit is documented in the Laravel 13 upgrade guide.

---

# Common Mistakes

- **Using Breeze for new projects (Laravel 12/13)**: Breeze is deprecated. New projects should use the appropriate Starter Kit for their frontend stack.
- **Security patches not applied**: Breeze controllers in the app directory do not receive automatic security updates. Monitor the Laravel changelog for auth-related security fixes and manually update controllers.
...
- Related: Laravel Jetstream (Fortify + Sanctum), Laravel Starter Kits (current)
- Advanced Follow-up: Legacy Breeze project migration to Fortify, Breeze to Starter Kit migration guide
