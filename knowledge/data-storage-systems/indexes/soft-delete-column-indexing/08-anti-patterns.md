# ECC Anti-Patterns â€” Soft Delete Column Indexing
---
## Metadata
| Field | Value |
|-------|-------|
...
### Related Skills
Design indexes that account for the automatic `WHERE deleted_at IS NULL` filter added by Eloquent's `SoftDeletes` trait â€” including `deleted_at` as the last column in composite indexes, or using partial indexes (PostgreSQL) to index only active rows for smaller, faster indexes.
...
---## Anti-Pattern 3: ### Indexing deleted_at alone
### Category
...
### Related Decision Trees
3-27-soft-delete-column-indexing decision trees in 07-decision-trees.md
