# ECC Anti-Patterns â€” Relationship Existence Filtering
---
## Metadata
| Field | Value |
|-------|-------|
...
### Related Skills
Use `has()` and `whereHas()` to query for records based on relationship existence or relationship conditions â€” `Post::has('comments')->get()` for posts with at least one comment, or `Post::whereHas('comments', fn($q) => $q->where('approved', true))` for filtered existence.
...
---## Anti-Pattern 3: ### Using has() when whereHas() is needed
### Category
...
### Related Decision Trees
2-6-relationship-existence-filtering decision trees in 07-decision-trees.md
