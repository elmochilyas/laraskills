# ECC Anti-Patterns â€” Subquery Ordering
---
## Metadata
| Field | Value |
|-------|-------|
...
### Related Skills
Use `orderBy()` with subqueries to sort results by derived values from related tables â€” `User::orderByDesc(Post::select('created_at')->whereColumn('user_id', 'users.id')->latest()->limit(1))` â€” enabling sorting by latest post date, highest order total, or other per-row computed values.
...
---## Anti-Pattern 3: ### Missing index on subquery column
### Category
...
### Related Decision Trees
2-9-subquery-ordering decision trees in 07-decision-trees.md
