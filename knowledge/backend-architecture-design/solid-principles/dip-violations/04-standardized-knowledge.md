...
# Core Concepts
This knowledge unit addresses key concerns related to SOLID principles in PHP: DIP violations in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Creating interfaces for every class â†’ over-abstraction (YAGNI)
...
- **Description**: Interface owned by infrastructure layer â†’ still violates DIP (high-level depends on low-level abstraction)
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Domain layer depending on Laravel contracts â†’ `Illuminate\Contracts` is still a framework dependency
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Constructor injection of primitives (strings, ints) â†’ use value objects or config objects
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: DIP only at service layer but not at presentation layer â†’ controllers still depend on concrete services
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Dependency injection, Interface design
Related: Hexagonal Architecture (DIP at architectural scale), Repository pattern (DIP for persistence), Anti-corruption layer (DIP for external systems)
...
When assisting with architecture decisions related to SOLID principles in PHP: DIP violations:
