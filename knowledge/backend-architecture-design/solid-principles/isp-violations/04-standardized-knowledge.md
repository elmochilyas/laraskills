...
Interface Segregation Principle states that no client should be forced to depend on methods it does not use. Large, "fat" interfaces force implementors to create empty or throwing methods for irrelevant functionality. In Laravel, common violations include monolithic repository interfaces with find/save/delete/search/export methods, and controller interfaces that mix disparate responsibilities. The correction involves splitting interfaces by role/client-specific needs â€” "role interfaces" that serve one consumer.

---

# Core Concepts
This knowledge unit addresses key concerns related to SOLID principles in PHP: ISP violations in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Interface explosion: one interface per method â†’ navigation nightmare
...
- **Description**: Role interfaces that mirror implementation structure â†’ technology-named, not role-named
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Over-splitting then needing to inject 5 interfaces â†’ constructor pollution
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Not using interface at all â†’ ISP violation by default (client depends on concrete class)
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Interface copied from another language's conventions (Java-style Repository interface)
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Interface basics in PHP
Related: SRP (classes â†’ interfaces), LSP (interface hierarchy), Repository pattern (interface design)
...
When assisting with architecture decisions related to SOLID principles in PHP: ISP violations:
