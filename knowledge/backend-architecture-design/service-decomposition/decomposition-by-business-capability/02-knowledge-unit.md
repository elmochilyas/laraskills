...
Two primary strategies drive service decomposition: business capabilities (what the business does â€” process-oriented) and DDD subdomains (core/supporting/generic â€” value-oriented). Business capability decomposition maps to organizational structure (Conway's Law). Subdomain decomposition identifies where competitive advantage lies. The strategies often converge but prioritize differently: capabilities focus on business processes; subdomains focus on business value and complexity. Hybrid approaches use subdomains for core capabilities and capability patterns for supporting/generic.

---

# Comparison

| Aspect | Business Capability | DDD Subdomain |
|--------|-------------------|---------------|
| Unit | Business process (Order Management) | Domain value (Core = competitive, Support = necessary, Generic = off-shelf) |
| Source | Org chart, business functions | Domain expert analysis, Event Storming |
| Focus | What business does | Where competitive advantage lies |
| Investment | Even per capability | Uneven (most in core) |
| Coupling | May span multiple subdomains | Aligned to bounded contexts |
| Best for | Conway's Law alignment, team ownership | Model integrity, strategic investment |

---

# Decomposition Strategy by Subdomain Type

| Subdomain Type | Decomposition Approach | Architecture |
|---------------|----------------------|--------------|
| Core | Custom service, rich domain model | DDD, Hexagonal, event-sourced if needed |
| Supporting | Custom but simpler, less investment | Transaction Script, layered |
| Generic | Off-the-shelf, minimal customization | Third-party service, SaaS |

---

# Common Mistakes

- Treating all subdomains as core â†’ overspending on CRUD areas
- Decomposing by technical layer (frontend, backend, DB) â†’ not aligned to business
- Capability decomposition ignoring subdomain value â†’ equal investment in commodity features
- Subdomain decomposition without organizational alignment â†’ team boundaries don't match
- Over-decomposition: 20 microservices for 5 capabilities â†’ distributed monolith
- Under-decomposition: one service for all capabilities â†’ regulated monolith

---

# Related Knowledge Units

**Prerequisites**: Strategic DDD, Business capability mapping | **Related**: Bounded contexts, Service boundaries, Module boundaries in monoliths, Conway's Law | **Advanced**: Event Storming for boundary discovery, Core domain charters, Investment allocation per subdomain

---

# Core Concepts

decomposition-by-business-capability is built on foundational concepts that govern its application in backend architecture.
...
In the context of service-decomposition, these concepts manifest through specific structural and behavioral rules that guide implementation decisions.

---

# Mental Models

Several mental models help reason about decomposition-by-business-capability effectively:

- **Layered Abstraction**: Think of decomposition-by-business-capability as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: decomposition-by-business-capability boundaries are best understood by mapping business capabilities — each cohesive business function maps to one architectural unit.
...
The internal mechanics of decomposition-by-business-capability involve several operational concerns:
...
Several established patterns relate to decomposition-by-business-capability:

| Pattern | Purpose | Application in service-decomposition |
|---------|---------|--------------------------|
...
These patterns are commonly used together to create robust decomposition-by-business-capability solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying decomposition-by-business-capability:
...
Common failure modes when applying decomposition-by-business-capability:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for decomposition-by-business-capability concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for decomposition-by-business-capability implementation.
