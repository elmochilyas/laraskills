...
Modular monolith starts as a single deployment unit but with clear module boundaries (like microservices without distribution). Each module has its own domain, database schema (separate tables, not separate databases), and communication via well-defined interfaces. This architecture is the recommended starting point for most applications — it provides microservices-like modularity without distributed system complexity. When the monolith's constraints become binding, individual modules can be extracted into services.
...
- Clear module boundaries (no cross-module direct dependencies)
- Module-specific database tables (no shared tables across modules)
- Module communication via interfaces, not directly
- Shared kernel (minimal common code — value objects, base types)
- Module autonomy: each module could become a service
...
- Shared database tables across modules → coupling at data level
- Modules calling each other's Eloquent models directly → bypasses interface
- Shared kernel growing too large → modules tightly coupled through shared code
- No interface between modules → cannot extract module without rewrite
- Premature module separation → wrong boundaries, frequent refactoring
- Modules not independently testable → module tests depend on other modules

---

# Related Knowledge Units

**Prerequisites**: Modular design, Bounded contexts | **Related**: Service decomposition, Microservices decomposition threshold, Domain modules | **Advanced**: Module communication patterns, Module extraction to service, Shared kernel governance

---

# Core Concepts

modular-monolith is built on foundational concepts that govern its application in backend architecture.
...
Several mental models help reason about modular-monolith effectively:

- **Layered Abstraction**: Think of modular-monolith as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: modular-monolith boundaries are best understood by mapping business capabilities � each cohesive business function maps to one architectural unit.
...
The internal mechanics of modular-monolith involve several operational concerns:
...
Several established patterns relate to modular-monolith:
...
These patterns are commonly used together to create robust modular-monolith solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying modular-monolith:
...
Common failure modes when applying modular-monolith:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for modular-monolith concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for modular-monolith implementation.
