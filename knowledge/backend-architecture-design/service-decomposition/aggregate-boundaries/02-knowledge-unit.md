...
Aggregates (from DDD tactical design) are consistency boundaries that group related entities and value objects under a root entity. Aggregate boundaries serve as natural decomposition units for services â€” each aggregate root can be a service boundary if transaction boundaries and consistency requirements align. The aggregate's invariant rules determine what belongs inside vs outside the boundary. Aggregates should be small (one entity + few VOs) and communicate via domain events for cross-aggregate operations.

---

# Aggregate Design Heuristics

- **Small aggregates**: reference other aggregates by ID, not by object reference
- **Consistency boundary**: all invariants maintained within the aggregate
- **Transaction scope**: one aggregate = one transaction
- **Eventual consistency**: between aggregates via domain events
- **Loading strategy**: load entire aggregate at once (eager)

---

# Aggregate â†’ Service Mapping

| Aggregate Characteristic | Service Boundary |
|-------------------------|-----------------|
| Single aggregate root | Single service owns it |
| Multiple related aggregates | Possible same service, check transaction scope |
| Independent aggregates | Separate services |
| Reference by ID only | Strong candidate for service split |

---

# Common Mistakes

- Giant aggregates with many entities â†’ performance issues, transaction conflicts
- Tiny aggregates with one field â†’ unnecessary fragmentation
- Cross-aggregate transactions â†’ distributed transaction complexity
- Aggregate referencing other aggregates by object â†’ coupling, loading overhead
- Ignoring aggregates in service decomposition â†’ services split by technical concerns

---

# Related Knowledge Units

**Prerequisites**: DDD tactical aggregates, Transaction boundaries | **Related**: Bounded contexts, Service boundaries, Transactional boundaries, Eventual consistency | **Advanced**: Aggregate design for performance, Eventual consistency between aggregates, Aggregate -> service extraction patterns

---

# Core Concepts

aggregate-boundaries is built on foundational concepts that govern its application in backend architecture.
...
Several mental models help reason about aggregate-boundaries effectively:

- **Layered Abstraction**: Think of aggregate-boundaries as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: aggregate-boundaries boundaries are best understood by mapping business capabilities — each cohesive business function maps to one architectural unit.
...
The internal mechanics of aggregate-boundaries involve several operational concerns:
...
Several established patterns relate to aggregate-boundaries:
...
These patterns are commonly used together to create robust aggregate-boundaries solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying aggregate-boundaries:
...
Common failure modes when applying aggregate-boundaries:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for aggregate-boundaries concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for aggregate-boundaries implementation.
