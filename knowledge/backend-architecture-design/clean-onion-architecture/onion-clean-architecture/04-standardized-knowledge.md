...
Clean Architecture (and its precursor Onion Architecture) organizes code into concentric rings where dependencies point inward â€” outer rings (frameworks, databases, UI) depend on inner rings (business rules), never the reverse. The dependency rule ensures that business logic is framework-independent, testable, and swappable. In Laravel, Clean Architecture requires careful boundary management: controllers are outermost (framework), use cases are inner ring, entities are innermost. The challenge is respecting the dependency rule while using Laravel's conveniences.

---

# Core Concepts
This knowledge unit addresses key concerns related to Onion Architecture / Clean Architecture dependency rule in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Inner ring depending on framework â†’ violates dependency rule
...
- **Description**: "Use Case" class that directly calls Eloquent â†’ use case depends on outer ring
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Entity with persistence annotations â†’ entity knows about DB
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Interface defined in outer ring, implemented in inner â†’ dependency direction reversed
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Boundary objects (DTOs) containing framework-specific code â†’ couples crossing boundary
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Over-compartmentalizing: empty use cases for simple CRUD â†’ more code, no benefit
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Dependency Inversion Principle, Hexagonal Architecture
Related: Hexagonal vs Clean differences, Boundary objects (DTOs, VOs), Use case granularity
...
When assisting with architecture decisions related to Onion Architecture / Clean Architecture dependency rule:
