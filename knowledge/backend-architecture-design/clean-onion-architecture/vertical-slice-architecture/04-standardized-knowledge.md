...
Vertical Slice Architecture organizes code by feature/use case rather than by technical layer. Each "slice" contains all layers needed for one feature â€” controller, request, service, model, view â€” creating vertical stacks through the system. This contrasts with layered architecture (horizontal layers for all features). Slices are independent, allowing each to use its own patterns without affecting others. The pattern is gaining traction in PHP/Laravel (2024-2026) for its pragmatic alignment with feature teams and its tolerance of controlled duplication.

---

# Core Concepts
This knowledge unit addresses key concerns related to Vertical Slice Architecture as emerging alternative in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Premature abstraction across slices â†’ creating shared service layer anyway (returns to layered)
...
- **Description**: No shared kernel at all â†’ inconsistent value objects, duplicate VOs
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Slices too granular â†’ one slice per HTTP method, not per feature
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Slices too coarse â†’ entire module in one slice (should be split)
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Communication between slices via shared database â†’ tight coupling
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Importing from other slice's internal code â†’ slice boundary violation
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Feature organization, Layered architecture
Related: Modular monolith (horizontal vs vertical split), Domain modules, Feature-based development
...
When assisting with architecture decisions related to Vertical Slice Architecture as emerging alternative:
