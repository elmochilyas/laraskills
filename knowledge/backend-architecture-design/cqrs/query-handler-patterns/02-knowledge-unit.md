...
Query handlers encapsulate data retrieval in dedicated classes, separating read logic from write logic. Unlike commands, queries are idempotent and return data without side effects. In Laravel, query handlers can use Eloquent, plain SQL, or read-optimized stores. The pattern enables distinct read models optimized for specific use cases, avoiding the "one model fits all" problem where a single entity class tries to serve both transactional writes and complex reads.
...
- Separation: query handlers don't call command handlers
- Idempotent: same query â†’ same result, no mutations

---

# Mental Models

- **Library Card Catalog**: Query handler knows where to find books efficiently
- **Data Reporter**: Query handler formats data for specific consumer
- **Read-Optimized Path**: Separate from transaction-heavy write path
...
|---------|---------------|----------|
| Query Class | Plain DTO with handler | Formal CQRS, explicit contracts |
| Repository Query Method | Repository with query methods | Simpler CQRS, less ceremony |
| Direct Service Method | Service class query methods | Level 0 CQRS (method separation only) |
| Cached Query | Query handler with caching | Read-heavy, slow-changing data |

---

# Common Mistakes

- Query handler modifying state â†’ violates CQRS
- Query returning entities instead of DTOs â†’ couples read models to ORM
- Query handler in same class as command handler â†’ mixes concerns
- Over-separating: query handler per field combination â†’ class explosion
- Not optimizing for read patterns â†’ query handler uses write-optimized models

---

# Related Knowledge Units

**Prerequisites**: CQRS basics, DTO patterns | **Related**: Read model strategies, Repository pattern, Caching strategies | **Advanced**: Query optimization, Materialized view handlers, Read model projections

---

# Internal Mechanics

The internal mechanics of query-handler-patterns involve several operational concerns:
...
Key architectural decisions when applying query-handler-patterns:
...
Common failure modes when applying query-handler-patterns:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for query-handler-patterns concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for query-handler-patterns implementation.
