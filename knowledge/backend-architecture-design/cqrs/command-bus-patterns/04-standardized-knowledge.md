...
Command bus dispatches command objects to their handlers, optionally through middleware pipelines. In Laravel, both the built-in Bus facade and queue system implement command bus patterns. The bus decouples command dispatch from execution, enabling middleware (logging, validation, transactions), queued execution, and audit trails. Laravel's bus supports sync dispatch (same process) and async dispatch (queue) with consistent middleware support.

---

# Core Concepts
This knowledge unit addresses key concerns related to Command bus patterns in PHP/Laravel context in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Command containing non-serializable data â†’ queue failures
...
- **Description**: Sync-only approach when async needed â†’ slow responses
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Not versioning command classes â†’ stale handlers after deploy
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Command in domain layer but handler in infrastructure â†’ dependency direction violation
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Command pattern, Message queuing
Related: CQRS maturity levels, Queue jobs, Middleware patterns
...
When assisting with architecture decisions related to Command bus patterns in PHP/Laravel context:
