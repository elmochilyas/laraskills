...
- Command: immutable DTO carrying data for an operation
- Handler: class that receives command and performs operation
- Bus: dispatcher that routes command to handler
- Middleware: pipeline wraps command execution with cross-cutting concerns
- Two buses: synchronous (immediate) and asynchronous (queue)

---

# Mental Models

- **Envelope**: Command is an envelope, handler opens and processes it
- **Post Office**: Bus routes command to correct handler like mail sorting
- **Assembly Line**: Middleware pipes add processing steps before handler executes
...
|---------|---------------|----------|
| Self-handling command | Command has handle() method | Simple cases, but couples command to handler |
| Separate handler class | Command + Handler class per command | CQRS alignment, handler injection |
| Queued command | ShouldQueue interface | Async processing, retry, backoff |
| Batch command | Bus::batch() | Multiple commands as unit |
| Middleware pipeline | Bus::pipeThrough() | Logging, validation, DB transactions |

---

# Common Mistakes

- Command containing non-serializable data â†’ queue failures
- Sync-only approach when async needed â†’ slow responses
- Not versioning command classes â†’ stale handlers after deploy
- Command in domain layer but handler in infrastructure â†’ dependency direction violation

---

# Related Knowledge Units

**Prerequisites**: Command pattern, Message queuing | **Related**: CQRS maturity levels, Queue jobs, Middleware patterns | **Advanced**: Idempotent command handling, Saga pattern, Command versioning

---

# Internal Mechanics

The internal mechanics of command-bus-patterns involve several operational concerns:
...
Key architectural decisions when applying command-bus-patterns:
...
Common failure modes when applying command-bus-patterns:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for command-bus-patterns concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for command-bus-patterns implementation.
