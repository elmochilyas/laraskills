...
CQRS should be applied selectively per bounded context, not as a system-wide architecture. Different contexts have different read/write symmetry. Invoices might benefit from CQRS Level 2 (separate read model for reporting), while User Registration remains Level 0 (simple CRUD). Selective application prevents overengineering while maximizing benefit where read/write asymmetry is high. The pattern is called "CQRS Islands" â€” each bounded context chooses its own CQRS depth.

---

# Core Concepts

- Context-specific CQRS depth: each bounded context independently
- Read/write asymmetry: high asymmetry â†’ higher CQRS level
- Business value: only apply where read/write separation provides tangible benefit
- Operational complexity: each CQRS level adds infrastructure cost
- Migration path: start Level 0, increase as needed
...
|-----------|------------------|--------|
| Simple CRUD, no reports | Level 0 | No benefit from separation |
| API with different read/write shapes | Level 1 | Different models for input/output |
| Reporting on same data | Level 1-2 | Read model optimized for reports |
| High-volume reads, low-volume writes | Level 2 | Separate read storage |
| Audit/legal requirements | Level 3 | Event-sourced commands |
| Independent team scaling | Level 4 | Separate deployment |

---

# Common Mistakes

- System-wide CQRS Level 3 â†’ massive overengineering for simple contexts
- No CQRS anywhere â†’ missing optimization opportunities
- Mixed levels without clear boundaries â†’ confusion about which level applies
- Starting at Level 3 â†’ unable to reduce complexity later

---

# Related Knowledge Units

**Prerequisites**: Bounded contexts, CQRS maturity levels | **Related**: Domain-Driven Design bounded contexts, Event sourcing selection criteria | **Advanced**: CQRS migration path per context, Mixed-consistency systems

---

# Mental Models

Several mental models help reason about selective-cqrs-application effectively:

- **Layered Abstraction**: Think of selective-cqrs-application as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: selective-cqrs-application boundaries are best understood by mapping business capabilities — each cohesive business function maps to one architectural unit.
...
The internal mechanics of selective-cqrs-application involve several operational concerns:
...
Several established patterns relate to selective-cqrs-application:
...
These patterns are commonly used together to create robust selective-cqrs-application solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying selective-cqrs-application:
...
Common failure modes when applying selective-cqrs-application:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for selective-cqrs-application concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for selective-cqrs-application implementation.
