...
Singleton ensures a class has exactly one instance and provides global access to it. In Laravel, the service container's singleton/scoped binding methods provide the same guarantee without the pattern's harmful side effects (hidden dependencies, global state, testability problems). The GoF Singleton is largely obsolete in Laravel â€” the container manages instance lifetime explicitly, making the pattern's self-managed static instance an anti-pattern. Understanding when single-instance semantics are appropriate (configuration, logging, stateless services) vs harmful (request-scoped state, mutable domain objects) is critical for correct Laravel architecture.

---

# Core Concepts
This knowledge unit addresses key concerns related to Singleton pattern in PHP/Laravel context in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Using `singleton()` for request-scoped state in Octane â†’ cross-request data contamination
...
- **Description**: Classic GoF Singleton in service classes â†’ impossible to mock in tests without static method mocking
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Resolving services inside `register()` â†’ accessing container before all providers registered
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Binding concrete class to itself instead of interface â†’ auto-resolution bypasses binding
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Singleton with mutable internal state â†’ unpredictable behavior in concurrent scenarios
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Dependency Injection, Service Container basics
Related: Factory pattern (creates instances), Registry pattern (service container)
...
When assisting with architecture decisions related to Singleton pattern in PHP/Laravel context:
