...
# Core Concepts
This knowledge unit addresses key concerns related to Decorator pattern in PHP/Laravel context in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Decorator modifying wrapped component's state â†’ unpredictable behavior with shared instances
...
- **Description**: Decorator ordering assumptions â†’ logging after caching misses cache hits
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Throwing exceptions from decorator without cleanup â†’ partially executed stack leaks state
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Decorator wrapping decorator with incompatible lifetime â†’ memory leaks in long-running processes
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Not delegating to parent â†’ decorator silently swallows behavior
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Composition vs Inheritance, Interface Segregation
Related: Chain of Responsibility (passes request vs adds behavior), Proxy (controls access vs adds behavior), Strategy (swaps algorithm vs wraps behavior)
...
When assisting with architecture decisions related to Decorator pattern in PHP/Laravel context:
