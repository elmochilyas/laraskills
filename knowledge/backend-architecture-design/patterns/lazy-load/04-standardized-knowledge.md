...
# Core Concepts
This knowledge unit addresses key concerns related to Lazy Load pattern in PHP/Laravel context in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: **N+1 in loops**: `foreach($users as $user) { $user->posts }` â†’ N+1 queries
...
- **Description**: **Lazy loading in serialization**: `toArray()` or `toJson()` triggers lazy load for included relations
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: **Conditional lazy loading in views**: Blade template accesses relation â†’ unpredictable queries
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: **Lazy loading disabled globally**: removes guard, production N+1 undetected
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: **Not using `load()`**: eager-loading after initial query is better than lazy for collections
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Eloquent relationships
Related: Proxy pattern (lazy proxy variant), N+1 detection strategies, Eager loading, LazyCollection vs Collection
...
When assisting with architecture decisions related to Lazy Load pattern in PHP/Laravel context:
