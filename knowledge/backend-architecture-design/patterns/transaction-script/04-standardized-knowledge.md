...
# Core Concepts
This knowledge unit addresses key concerns related to Transaction Script (Fowler) in PHP/Laravel context in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Transaction Script that grows beyond 100 lines â†’ breaking SRP
...
- **Description**: Duplicating validation logic across multiple scripts â†’ inconsistent rules
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Mixing UI concerns in transaction script â†’ controller doing business logic
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Transaction script that assumes specific UI â†’ cannot reuse for API + web
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: No separation of concerns â†’ script does validation, formatting, AND DB access
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: MVC controller pattern
Related: Domain Model (opposite end of complexity spectrum), Service Layer (organizes transaction scripts), Table Module (tabular logic organization)
...
When assisting with architecture decisions related to Transaction Script (Fowler) in PHP/Laravel context:
