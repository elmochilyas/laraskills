...
# Core Concepts
This knowledge unit addresses key concerns related to Service Layer (Fowler) in PHP/Laravel context in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Fat service with 20+ methods â†’ god object, SRP violation
...
- **Description**: Service that knows about HTTP (request, session, auth facade) â†’ couples domain to web
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Service returning HTTP response objects â†’ cannot reuse for CLI/queue
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Anemic service: just calls repository with no logic â†’ unnecessary layer
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Service with mixed concerns: reporting + CRUD + email in same class
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Dependency injection, Controller patterns
Related: Transaction Script (service as organizer), Domain Model (service as facade), Action Domain classes, CQRS Service layer
...
When assisting with architecture decisions related to Service Layer (Fowler) in PHP/Laravel context:
