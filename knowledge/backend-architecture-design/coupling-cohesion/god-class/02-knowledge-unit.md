...
God class is a class that knows too much or does too much â€” accumulating responsibilities across multiple concerns. In Laravel, Eloquent models are the most common god objects: User models that handle authentication, profile management, billing, notifications, permissions, settings, and reporting. Eloquent's design makes this tempting (model has ready access to DB, events, relationships), but as models grow, they violate SRP, become untestable, and create a bottleneck for changes.
...
|--------|-----------|------------|
| Lines of code | >500 lines | Likely god class |
| Number of methods | >20 public methods | Too many responsibilities |
| Method categories | Auth + billing + notifications + profile | Cross-domain responsibilities |
| Dependencies | >5 injected services | Does too much |
| Traits used | >3 traits | Accumulating behavior |
| Accessors/Mutators | >10 | Model is too intelligent |
| Event listeners | >5 model events | Too many side effects |

---

# Common God Class Sources in Laravel

- **User model**: authenticatable, billable, notifiable, has roles, has profile, has settings
- **Order model**: order logic + payment + shipping + notifications + status machine
- **Product model**: catalog + inventory + pricing + reviews + recommendations
- **Service classes**: CRUD + reporting + export + email + notifications

---

# Remediation

1. Extract authentication: separate `AuthenticatedUser` concept
2. Extract billing: `BillableUser` or separate `Subscription` model
3. Action classes: each use case gets its own class
4. Domain services: cross-model logic moves to services
5. Single-responsibility models: Model stays focused on persistence + core relationships
6. Read models: reporting queries use dedicated read models, not the write model

---

# Related Knowledge Units

**Prerequisites**: SRP, Cohesion | **Related**: Big Ball of Mud (god classes contribute), Premature abstraction (over-reaction), Anemic domain model (under-reaction) | **Advanced**: Model decomposition patterns, Responsibility extraction, Feature-based organization

---

# Core Concepts

god-class is built on foundational concepts that govern its application in backend architecture.
...
Several mental models help reason about god-class effectively:

- **Layered Abstraction**: Think of god-class as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: god-class boundaries are best understood by mapping business capabilities — each cohesive business function maps to one architectural unit.
...
The internal mechanics of god-class involve several operational concerns:
...
Several established patterns relate to god-class:
...
These patterns are commonly used together to create robust god-class solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying god-class:
...
Common failure modes when applying god-class:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for god-class concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for god-class implementation.
