...
# Core Concepts
This knowledge unit addresses key concerns related to God class detection (Eloquent models as god objects) in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Over-applying abstractions without concrete need
- **Cause**: Anticipating future requirements that never materialize
- **Consequence**: Increased complexity, reduced readability, maintenance burden
- **Better**: Apply patterns only when concrete variation points emerge
...
Prerequisites: SRP, Cohesion
Related: Big Ball of Mud (god classes contribute), Premature abstraction (over-reaction), Anemic domain model (under-reaction)
...
When assisting with architecture decisions related to God class detection (Eloquent models as god objects):
