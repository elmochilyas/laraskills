...
Cohesion measures how closely the responsibilities of a module are related. Higher cohesion is better — elements within a module should contribute to a single, well-defined purpose. The cohesion spectrum (coincidental → logical → temporal → procedural → communicational → sequential → functional) ranges from worst to best. In Laravel, low cohesion appears as "Utility" classes (logical cohesion), god models (coincidental/functional mix), and service classes doing unrelated operations. Measurement helps identify classes that should be split.

---

# Cohesion Spectrum

| Type | Level | Laravel Example | Assessment |
|------|-------|-----------------|------------|
| Functional | Strongest | `SendWelcomeEmail` action class | Single purpose, cannot be split |
| Sequential | Strong | Pipeline stages (middleware chain) | Output of one is input to next |
| Communicational | Strong | Report generator (fetch + format + export) | Same data, different operations |
| Procedural | Moderate | Controller method (validate → save → email) | Time-ordered operations |
| Temporal | Moderate | Maintenance tasks (clear cache, generate sitemap) | Same time, unrelated logic |
| Logical | Weak | `Utilities` class, `Helpers` | Category grouping of unrelated functions |
| Coincidental | Weakest | `Miscellaneous` methods | Arbitrary grouping |

---

# Detection Heuristics

| Indicator | Low Cohesion Sign | Correction |
|-----------|-------------------|------------|
| Class name "Utility", "Helper", "Manager" | Logical/coincidental cohesion | Split into focused classes |
| Class with "and" in description | Multiple unrelated responsibilities | Extract each responsibility |
| Constructor with 5+ dependencies | Class does too much | Split by responsibility |
| Class changed for different reasons | Multiple variation axes | Split per variation axis |
| Method categories in docblock | Multiple responsibility groups | Extract each group |
...
|--------|-----------------|-----|
| Lack of Cohesion of Methods (LCOM) | How many methods share fields | Low LCOM = low cohesion |
| Relational Cohesion (H) | Internal connectivity | High = more connected |
| Method overlap ratio | Field sharing between methods | More sharing = higher cohesion |

---

# Common Mistakes

- Treating cohesion as boolean → it's a spectrum, not pass/fail
- Functional cohesion for everything → not all classes need to be single-method
- High cohesion at expense of coupling → splitting classes increases coupling
- Measuring without context → metrics without understanding class purpose
- Ignoring package-level cohesion → focusing only on class cohesion

---

# Related Knowledge Units

**Prerequisites**: Coupling types, Class design | **Related**: Coupling measurement, SRP, Modularity metrics, Class splitting techniques | **Advanced**: LCOM calculation, Cohesion in package design (CRP, REP), Cohesion-coupling tradeoff matrix

---

# Core Concepts

cohesion-types-measurement is built on foundational concepts that govern its application in backend architecture.
...
Several mental models help reason about cohesion-types-measurement effectively:

- **Layered Abstraction**: Think of cohesion-types-measurement as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: cohesion-types-measurement boundaries are best understood by mapping business capabilities � each cohesive business function maps to one architectural unit.
...
The internal mechanics of cohesion-types-measurement involve several operational concerns:
...
Several established patterns relate to cohesion-types-measurement:
...
These patterns are commonly used together to create robust cohesion-types-measurement solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying cohesion-types-measurement:
...
Common failure modes when applying cohesion-types-measurement:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for cohesion-types-measurement concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for cohesion-types-measurement implementation.
