...
Event bus distributes events from producers to consumers. Two fundamental patterns exist: in-process (synchronous, within same process) and message broker (asynchronous, across processes/services). In Laravel, the built-in event system is in-process; queue-based events bridge to message brokers. The choice determines consistency guarantees, fault tolerance, and scaling characteristics. Mixing both in the same application is common â€” in-process for domain events within a context, message broker for integration events across contexts.

---

# Core Concepts
This knowledge unit addresses key concerns related to Event bus patterns (in-process vs message broker) in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: In-process for slow operations â†’ blocks response time
...
- **Description**: Message broker for every event â†’ unnecessary complexity for local side effects
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: No dead letter handling â†’ failed events lost forever
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Assuming message broker guarantees ordering â†’ depends on configuration
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: In-process events in long-running processes â†’ memory leaks if listeners accumulate
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Queue events without idempotency â†’ duplicate processing on retry
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Event-driven architecture basics, Laravel events/queues
Related: Outbox pattern, Dead letter handling, Domain vs integration events
...
When assisting with architecture decisions related to Event bus patterns (in-process vs message broker):
