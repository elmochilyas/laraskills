...
Events are persistent contracts â€” once written to an event store or published to a broker, old events coexist with new ones. Schema evolution strategies ensure backward compatibility so consumers don't break when event formats change. Key strategies: forward-only changes (add optional fields), versioned schemas (multiple event versions), and upcasting (transform old events to new format). In Laravel, Spatie's event sourcing supports event versioning via event classes per version.
...
|----------|----------|----------|------|
| Additive changes only | Add optional fields, never remove | Simple evolution, no breaking changes | Schema bloat over time |
| Versioned event classes | New class per version (OrderPlacedV1, OrderPlacedV2) | Complex schemas, breaking changes | Class proliferation |
| Upcasting | Transform old events to new format on read | Transparent to consumers | Upcaster maintenance |
| Schema registry | Central registry of event schemas (Avro, JSON Schema) | Multi-consumer systems | Infrastructure dependency |
| Tolerant reader | Consumers ignore unknown fields | Decoupled evolution | Consumer doesn't validate complete schema |

---

# Principles

1. **Backward compatibility**: new event format should be readable by old consumers
2. **Additive changes**: adding fields is safe; removing/renaming is breaking
3. **Consumer-driven contracts**: notify consumers before schema changes
4. **Schema registry**: for cross-service events, register schemas centrally
5. **Upcasters**: transform historical events to current schema

---

# Common Mistakes

- No versioning strategy from day 1 â†’ cannot evolve events after production
- Removing fields instead of deprecating â†’ immediate consumer breakage
- Renaming fields â†’ consumers looking for old name get null
- Changing field types â†’ type errors in consumers
- Upcasting in projection instead of in event store read â†’ upcasters scattered everywhere
- Events in shared namespace with no version â†’ can't distinguish old from new

---

# Related Knowledge Units

**Prerequisites**: Event sourcing, Schema design | **Related**: Upcaster pattern, Projection rebuild, Consumer-driven contracts | **Advanced**: Schema registry (Avro, JSON Schema), Multi-version event handling, Tolerant reader pattern

---

# Core Concepts

event-versioning-schema-evolution is built on foundational concepts that govern its application in backend architecture.
...
Several mental models help reason about event-versioning-schema-evolution effectively:

- **Layered Abstraction**: Think of event-versioning-schema-evolution as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: event-versioning-schema-evolution boundaries are best understood by mapping business capabilities — each cohesive business function maps to one architectural unit.
...
The internal mechanics of event-versioning-schema-evolution involve several operational concerns:
...
Several established patterns relate to event-versioning-schema-evolution:
...
These patterns are commonly used together to create robust event-versioning-schema-evolution solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying event-versioning-schema-evolution:
...
Common failure modes when applying event-versioning-schema-evolution:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for event-versioning-schema-evolution concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for event-versioning-schema-evolution implementation.
