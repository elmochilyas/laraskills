...
| Consumer | Same-context listeners | Other context services |
| Schema | Internal, can change freely | Published, versioned contract |
| Transport | In-process bus (Laravel events) | Message broker (queue, Kafka, RabbitMQ) |
| Example | OrderWasCreated (generates invoice PDF) | OrderPlaced (notifies shipping context) |
| Language | Ubiquitous language | Published contract language |

---

# Mental Models

- **Company Memo**: Internal memo (domain event) vs press release (integration event)
- **Org Chart**: Department decisions (domain) vs cross-department announcements (integration)

---

# Distinction Heuristics

- Ask: "Is this event consumed only within its bounded context?" â†’ Domain event
- Ask: "Would other contexts care about this?" â†’ Integration event
- Ask: "Can the event schema change without external coordination?" â†’ Domain event
- Translation often needed: Domain event â†’ Integration event via adapter at context boundary

---

# Common Mistakes

- Publishing domain events directly as integration events â†’ external schema coupled to internal model
- Integration events used within same context â†’ unnecessary message broker overhead
- Domain events never published as integration events â†’ other contexts don't know about changes
- Reusing same event class for both â†’ confusion about schema stability
- Not translating between domain and integration formats â†’ leaky context boundary

---

# Related Knowledge Units

**Prerequisites**: Bounded contexts, Domain events | **Related**: Anti-corruption layer (translation at boundaries), Event bus (transport differences), Context mapping | **Advanced**: Event translation strategies, Multi-schema event evolution

---

# Internal Mechanics

The internal mechanics of domain-vs-integration-events involve several operational concerns:
...
Several established patterns relate to domain-vs-integration-events:
...
These patterns are commonly used together to create robust domain-vs-integration-events solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying domain-vs-integration-events:
...
Common failure modes when applying domain-vs-integration-events:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for domain-vs-integration-events concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for domain-vs-integration-events implementation.
