...
Domain events record something meaningful that happened in the domain (within a bounded context), while integration events communicate state changes across bounded context boundaries. This distinction is critical for maintaining bounded context autonomy. Domain events are consumed within their context for side effects; integration events are published for external consumption. Mixing them causes coupling between contexts and violates bounded context boundaries.

---

# Core Concepts
This knowledge unit addresses key concerns related to Domain events vs integration events distinction in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Publishing domain events directly as integration events â†’ external schema coupled to internal model
...
- **Description**: Integration events used within same context â†’ unnecessary message broker overhead
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Domain events never published as integration events â†’ other contexts don't know about changes
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Reusing same event class for both â†’ confusion about schema stability
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Not translating between domain and integration formats â†’ leaky context boundary
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Bounded contexts, Domain events
Related: Anti-corruption layer (translation at boundaries), Event bus (transport differences), Context mapping
...
When assisting with architecture decisions related to Domain events vs integration events distinction:
