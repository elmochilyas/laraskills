...
Pure Fabrication assigns responsibility to a class that does NOT represent a domain concept when assigning it to a domain class would violate Low Coupling or High Cohesion. These are "made up" classes that solve technical or architectural problems without existing in the domain vocabulary. In Laravel, Repositories, Service classes, DTOs, Factories, and Action classes are all Pure Fabrications. They are essential for clean architecture but should be used judiciously â€” not as a default for every responsibility.

---

# Core Concepts
This knowledge unit addresses key concerns related to GRASP: Pure Fabrication in the context of backend architecture and design for PHP/Laravel applications.
...
- **Description**: Everything as Pure Fabrication â†’ no rich domain model, procedural code
...
- **Description**: Pure Fabrication with business logic â†’ logic lives outside domain, not reusable
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Pure Fabrication named after technical pattern, not role â†’ uncommunicative naming
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit

- **Description**: Not using Pure Fabrication when appropriate â†’ domain classes polluted with infrastructure concerns
- **Cause**: Lack of awareness or discipline in applying the pattern correctly
- **Consequence**: Reduced code quality, maintainability issues, or system fragility
- **Better**: Follow established patterns and practices documented in this knowledge unit
...
Prerequisites: Information Expert, Low Coupling, High Cohesion
Related: Repository (canonical Pure Fabrication), Service Layer, DTO patterns, Action classes
...
When assisting with architecture decisions related to GRASP: Pure Fabrication:
