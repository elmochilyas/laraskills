...
Indirection assigns responsibility to an intermediate object to mediate between other components, reducing direct coupling. In Laravel, indirection is everywhere: the service container mediates between service consumers and implementations, events mediate between producers and listeners, repositories mediate between domain and data sources. The pattern increases flexibility at the cost of additional indirection layers.

---

# Core Concepts

- Mediation: intermediate object handles communication between components
- Loose coupling: components don't know each other directly
- Polymorphism: intermediate can vary without affecting either side
- Types: events (producerâ†’listener), container (consumerâ†’provider), adapter (clientâ†’service)

---

# Indirection in Laravel

| Mechanism | Couples | Decouples | Benefit |
|-----------|---------|-----------|---------|
| Service Container | Container | Implementation from consumer | Swappable implementations |
| Events | Dispatcher | Producers from listeners | Add listeners without changing producers |
| Repository | Repository Interface | Domain from data source | Swap storage without domain changes |
| Middleware | Pipeline | Middleware from each other | Compose middleware independently |
| Adapter | Adapter Interface | Application from vendor | Change vendors without app changes |

---

# Common Mistakes

- Unnecessary indirection: adding layers where no decoupling benefit â†’ complexity without value
- Leaky indirection: intermediate object exposes underlying component's types â†’ defeats purpose
- Indirection hidden from developer â†’ "magic" behavior, hard to trace
- Over-indirection: 5 layers of abstraction for simple operation â†’ performance + navigation cost
- Not using indirection at natural boundaries â†’ high coupling, hard to change

---

# Related Knowledge Units

**Prerequisites**: Low Coupling, Facade pattern | **Related**: Pure Fabrication (indirection mechanisms are often Pure Fabrications), Mediator pattern, Event-driven architecture | **Advanced**: Indirect layer performance analysis, When indirection is justified

---

# Mental Models

Several mental models help reason about indirection effectively:

- **Layered Abstraction**: Think of indirection as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: indirection boundaries are best understood by mapping business capabilities — each cohesive business function maps to one architectural unit.
...
The internal mechanics of indirection involve several operational concerns:
...
Several established patterns relate to indirection:
...
These patterns are commonly used together to create robust indirection solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying indirection:
...
Common failure modes when applying indirection:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for indirection concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for indirection implementation.
