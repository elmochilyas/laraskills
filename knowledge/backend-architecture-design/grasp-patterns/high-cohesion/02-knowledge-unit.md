...
High Cohesion keeps related responsibilities together within a class or module. High cohesion means the elements of a class are strongly related and focused on a single purpose. In Laravel, cohesion is achieved through focused action classes, single-responsibility services, and well-bounded modules. High cohesion enables understandability, maintainability, and reusability â€” a class that does one thing well is easier to test, change, and reason about.

---

# Core Concepts

- Cohesion spectrum: coincidental (worst) â†’ logical â†’ temporal â†’ procedural â†’ communicational â†’ sequential â†’ functional (best)
- Functional cohesion: all elements contribute to a single well-defined function
- High cohesion = strong internal relatedness
- High cohesion = low coupling (balance required)
- Cohesion at multiple levels: method, class, package, module

---

# Cohesion Levels

| Level | Description | Laravel Example |
|-------|-------------|-----------------|
| Functional | Single well-defined function | `SendWelcomeEmail` action class |
| Sequential | Output of one â†’ input of next | Pipeline stages |
| Communicational | Same data, different operations | Report generator |
| Procedural | Time-ordered operations | Controller method |
| Temporal | Same time, unrelated | Maintenance tasks |
| Logical | Category grouping (worst) | `Helpers` class |
| Coincidental | Arbitrary (worst) | `Miscellaneous` |

---

# Common Mistakes

- Utils/Helpers classes â†’ coincidental cohesion
- God classes with unrelated methods â†’ zero cohesion
- Too many small classes (over-splitting) â†’ high cohesion but navigation overhead
- Sacrificing cohesion for reusability â†’ class does too much to "be reusable"
- Class named "Manager" â€” usually low cohesion

---

# Related Knowledge Units

**Prerequisites**: SRP, Class design | **Related**: Low Coupling (balance), God class (low cohesion), Modular design, Functional cohesion | **Advanced**: Cohesion metrics, LCOM measurement, Package cohesion principles (CRP, REP)

---

# Mental Models

Several mental models help reason about high-cohesion effectively:

- **Layered Abstraction**: Think of high-cohesion as onion-like layers where inner layers know nothing about outer layers. Each layer provides a level of indirection that simplifies reasoning about the system.
- **Business Capability Mapping**: high-cohesion boundaries are best understood by mapping business capabilities — each cohesive business function maps to one architectural unit.
...
The internal mechanics of high-cohesion involve several operational concerns:
...
Several established patterns relate to high-cohesion:
...
These patterns are commonly used together to create robust high-cohesion solutions in Laravel applications.

---

# Architectural Decisions

Key architectural decisions when applying high-cohesion:
...
Common failure modes when applying high-cohesion:
...
- **Laravel Framework**: Core service container, event system, queue infrastructure provide foundational support for high-cohesion concepts.
...
- **Spatie Packages**: Various Spatie packages (laravel-event-sourcing, laravel-queueable-actions) provide building blocks for high-cohesion implementation.
