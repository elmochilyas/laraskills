...
| Dependencies | Pest/PHPUnit fundamentals, Database testing lifecycle |
| Related KUs | CI/CD pipeline integration, Parallel sharding in CI, Matrix CI testing |
| Source | domain-analysis.md K015 |
...
- **Match worker count to available CPU cores minus 1**: Reason: leaves one core for OS/IO operations. Oversubscribing causes context switching overhead.
- **Always use `RefreshDatabase` or database isolation in parallel mode**: Reason: without isolation, parallel workers write to the same database, causing random test failures.
- **Profile with `--profile` to identify file size imbalances**: Reason: one large test file limits total wall time. Split large files for balanced distribution.
- **Use 4 workers for GitHub Actions hosted runners**: Reason: default runners have 2 CPUs. 4 workers is optimal for IO-bound tests.
- **Run coverage in a separate sequential pass**: Reason: parallel coverage merges are complex and error-prone.
- **Configure `max_connections` for MySQL parallel testing**: Reason: each worker needs 1+ database connections. Set to `worker_count * 2 + 20`.
- **Use `ParallelTesting` facade for custom per-worker setup**: Reason: `setUpProcess()`, `tearDownProcess()` hooks enable database creation and resource allocation.
...
- Consequence: Workers share a single database causing random failures
...
- Better: Profile with `--profile` to identify slow files.
...
- Consequence: Workers fail with "Too many connections" error
...
php artisan test --parallel  # auto-detect CPU count
...
ParallelTesting::tearDownProcess(function (int $token) {
    DB::statement("DROP DATABASE IF EXISTS myapp_test_{$token}");
});
```
...
- Matrix CI testing (PHP x DB variants)
