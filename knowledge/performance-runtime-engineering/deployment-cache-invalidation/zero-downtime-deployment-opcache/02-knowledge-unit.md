...
- **Load balancer orchestration**: 1) Mark server as draining (stop sending new connections), 2) Wait for in-flight requests to complete (10-30s), 3) Deploy code, 4) Reload PHP-FPM gracefully, 5) Warm OpCache (hit critical endpoints), 6) Health check passes, 7) Mark server as active (resume traffic).
- **OpCache pre-warming**: After reload, critical endpoints may be slow. Run a warm-up script: or url in / /api/health /api/users; do curl -s -o /dev/null http://localhost; done. Ensures OpCache is populated before user traffic resumes.
...
**Pre-warm script**: cachetool opcache:warm --urls=/api/health,/home,/products ? hits multiple endpoints to populate OpCache. Run after reload but before re-enabling traffic. Configurable endpoints and timeouts.

---

# Common Mistakes

**Skipping warm-up**: After reload with validate_timestamps=0, OpCache is empty. First request recompiles all files for that path. Users see 3-5s response times. Always warm before re-enabling traffic.
...
PHP-FPM Graceful Reload | OpCache Reset Strategies | Blue-Green Deployment
