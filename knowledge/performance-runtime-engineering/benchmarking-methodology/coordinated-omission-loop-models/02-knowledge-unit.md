...
Knowledge Unit: Coordinated Omission — Closed-Loop (wrk) vs Open-Loop (wrk2) Models
...
- **Closed-loop**: Tool waits for response ? sends next request. During overload, tool sends fewer requests ? measured throughput drops but latency stays apparently low ? underestimates real latency by 30-60%.
- **Open-loop**: Tool sends requests at a fixed rate (e.g., 1000 RPS). If the server can only handle 800 RPS, the remaining 200 requests accumulate queue delay ? measured latency accurately reflects user experience.
- **Example**: 1000 RPS target, server handles 800 RPS at 50ms. Closed-loop: reports 800 RPS at 50ms. Open-loop: reports 800 RPS at 50ms + 20% of requests at 500ms (queued). Only open-loop shows the true user impact.
...
**Correct benchmarking protocol**: 1) Warm-up, 2) Measure baseline latency at low rate (open-loop), 3) Gradually increase rate until p99 latency increases 2x over baseline, 4) Report both throughput and latency at each rate.

---

# Common Mistakes

**Trusting ab/wrk latency numbers**: These tools systematically underestimate tail latency by 30-60% during saturation. Always verify tail latency measurements with an open-loop tool.
...
wrk/wrk2 Usage and Lua Scripting | Benchmarking Concepts | HDR Histogram Analysis
