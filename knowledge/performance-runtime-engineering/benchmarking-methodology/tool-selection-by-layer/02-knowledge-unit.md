...
Knowledge Unit: Tool Selection by Layer — Protocol Load (k6, JMeter), Network Edge (wrk, Vegeta), Browser
...
- **k6**: JavaScript-scriptable load testing. Best for user-journey testing. Stages (ramp-up/steady/ramp-down), thresholds (p95<200ms), checks (status=200), custom metrics. Open source + cloud.
- **wrk/wrk2**: Multi-threaded HTTP benchmarking. Best for raw throughput and latency measurement. wrk2 adds constant-RPS mode to avoid coordinated omission. Lua scripting for request customization.
- **Vegeta**: Go-based HTTP benchmarking. Simple CLI. Best for ad-hoc throughput tests and CI integration. Constant-rate or targeted-RPS modes.
- **JMeter/Gatling**: Java-based. Best for complex enterprise scenarios with assertions, correlation, and distributed load generation.
...
**Using Apache Bench (ab) for production benchmarking**: ab is single-threaded, closed-loop, and cannot simulate realistic traffic. It overestimates capacity by 30-50%. Use wrk2 (open-loop) or k6 for production-realistic results.
...
wrk/wrk2 Usage and Lua Scripting | k6 Scripting Thresholds and Stages | Benchmarking Concepts
