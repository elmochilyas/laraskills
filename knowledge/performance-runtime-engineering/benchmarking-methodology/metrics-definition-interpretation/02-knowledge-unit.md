...
Knowledge Unit: Metrics Definition and Interpretation — Throughput, Latency Distribution, Memory, CPU
...
- **Throughput (RPS)**: Peak sustainable requests per second. Measured at plateau, not instantaneous. Diminishing returns as concurrency increases.
...
- **Memory metrics**: Worker RSS (resident set size — physical memory per process), total PHP memory, OpCache usage, GC buffer size. Used for capacity planning.
- **CPU utilization**: User + system + iowait. Under 70% user is healthy. Over 90% user indicates CPU-bound bottleneck. High iowait indicates I/O bottleneck.

---

# Patterns

**Metric correlation**: When throughput plateaus but p50 latency is low, bottleneck is elsewhere (network, database). When p99 latency diverges from p50, tail latency is driven by queuing or garbage collection.

---

# Common Mistakes

**Ignoring memory metrics in benchmarks**: A configuration that achieves 20% higher throughput but uses 40% more memory may reduce overall capacity. Always report resource-per-request.
...
Benchmarking Concepts | Tool Selection by Layer | HDR Histogram Analysis
