...
Knowledge Unit: Performance Regression Detection — CI Baseline Comparison, Threshold-Based Pass/Fail
...
- **Baseline establishment**: Collect 30+ benchmark runs from the last-known-good commit (release tag or main branch baseline). Calculate mean and standard deviation for each metric.
...
**Multi-stage detection**: 1) Hard threshold fail (immediate PR block), 2) Statistical regression warning (review within 24h), 3) Gradual drift alert (trend over 7 days). Each stage triggers different escalation.

---

# Common Mistakes

**Comparing against a single previous run**: One benchmark run has ~3-8% variance. Compare against a baseline of 30+ runs (recent window). Any single-run comparison produces false positives.
...
CI Integration and Baseline Comparison | Statistical Significance in Benchmarking | SLO Definition and Error Budgets
