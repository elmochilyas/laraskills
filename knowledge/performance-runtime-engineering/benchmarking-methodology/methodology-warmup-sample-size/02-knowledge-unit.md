...
Knowledge Unit: Methodology — Warm-up Phases, Sample Size Requirements, Environment Control
...
- **Warm-up**: First N requests after server start are significantly slower (OpCache cold, JIT compiling, DB connections establishing). Run 30-60s of traffic before recording measurements. Discard warm-up data.
- **Sample size**: Statistical confidence depends on sample count. For p95: 1000 samples (95% CI ± 1-2%). For p99: 10,000 samples. For p99.9: 100,000+ samples.
- **Environment control**: Run on dedicated (non-shared) hardware. Pin CPU frequency to prevent turbo boost variance. Use fixed dataset (no random data generation during test). Isolate network to prevent external interference.
- **Comparative benchmarks**: A/B comparisons (PHP version, config change) must control all variables except the one being tested. Run both configurations in the same environment, same dataset, same time window.

---

# Common Mistakes

**No warm-up phase**: The first 30 seconds of benchmark data are typically 20-50% slower than steady state. Including warm-up data underestimates real-world capacity.
...
Benchmarking Concepts | Statistical Significance in Benchmarking | CI Integration and Baseline Comparison
