...
- **gc_collect_cycles() cost**: 50-500μs per full cycle depending on root buffer size and object graph complexity
- **Per-iteration overhead**: Calling GC after every loop iteration in a 1000-iteration loop adds 50-500ms of total GC time
- **Batch boundary pattern**: Call after processing a batch of items (every 100 queue jobs, after each HTTP request in Octane) — amortizes GC cost across meaningful units of work
- **gc_status() pre/post**: `$before = gc_status()['root_buffer_entries']; gc_collect_cycles(); $collected = gc_status()['collected'];` — monitor collection effectiveness

---

# Patterns

**Octane worker pattern**: Call `gc_collect_cycles()` after each request to prevent inter-request cycle accumulation. The default Laravel Octane middleware handles this. In custom workers, implement equivalent cleanup.

---

# Performance Considerations

- Root buffer reaching 10,000 triggers automatic GC (stop-the-world pause). Proactive calling at controlled times prevents unpredictable pauses.
- gc_collect_cycles() returns number of collected cycles. If consistently 0, cycles are not forming — stop calling it.
- PHP 8.5 GC improvements reduce false positives from static fake closures and Enum singletons, reducing unnecessary collections.

---

# Common Mistakes

**Calling gc_collect_cycles() before every database query**: GC has nothing to do with database memory. Calls outside the root buffer threshold waste CPU. Only call when you suspect cycle accumulation.

---

# Related Knowledge Units

Cyclic GC Algorithm | GC Enable/Disable Patterns | GC Telemetry and Root Buffer Monitoring
