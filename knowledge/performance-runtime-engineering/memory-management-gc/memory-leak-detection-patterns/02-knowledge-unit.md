...
**Assuming pm.max_requests solves memory leaks**: Worker recycling masks leaks but does not fix them. At scale, the cost of constant worker recycling (~200ms spawn overhead per worker restart) adds up. Fix the leak, don't just recycle around it.
...
GC Telemetry and Root Buffer Monitoring | Memory Drift Detection and Mitigation | PM Max Requests Tuning
