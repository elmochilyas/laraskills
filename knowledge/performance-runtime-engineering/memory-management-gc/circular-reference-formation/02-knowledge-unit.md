...
- **Direct cycle**: `$parent->child = $child; $child->parent = $parent;` — both objects have refcount=1 even after `unset($parent, $child)`. The GC must collect them.
- **Self-reference**: `$obj->self = $obj;` — refcount never drops below 1. Common in singleton patterns.
- **Event listener accumulation**: `$subject->listeners[] = function() use ($subject);` — the closure captures `$subject`, creating a cycle through the closure's scope. Accumulates over time in long-running processes.
...
**Cycle detection**: If `memory_get_usage()` increases across requests without corresponding data growth, suspect circular reference accumulation. Use `gc_status()` to check root buffer size.

---

# Common Mistakes

**Assuming unset() frees all memory**: `unset($a)` only decrements refcount. If `$a` is part of a cycle, it remains in memory until the GC runs. For deterministic cleanup, `gc_collect_cycles()` can be called explicitly.
...
Cyclic GC Algorithm | WeakReference API Usage | Memory Leak Pattern Catalog
