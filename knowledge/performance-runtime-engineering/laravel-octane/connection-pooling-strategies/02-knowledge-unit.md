...
- **Persistent connections in Octane**: Octane creates connections at worker start. Each request within the same worker reuses the same PDO/Redis connection. No TCP handshake per request (~0.5-2ms saved).
...
- **Connection budgeting**: `worker_count × connections_per_worker = database_max_connections × 0.8`. With 8 workers and 3 connections each (MySQL + Redis + HTTP), need 24 DB connections.
- **Read/write splitting**: Separate read and write connections. Read replicas can serve more concurrent queries without exhausting the write-primary connection pool.

---

# Patterns

**Connection pool health monitoring**: After Octane deployment, monitor database connection count. Each worker opens connections once and never closes them. If connections exhaust the database pool, reduce worker count or increase DB max_connections.

---

# Common Mistakes

**Octane + transactions across requests**: If a transaction is left open at request end (missing commit/rollback), the next request inherits the transaction context. All queries in the next request are part of the stale transaction.
...
State Management and Leak Prevention | Worker Configuration by Driver | Capacity Planning Safety Margins
