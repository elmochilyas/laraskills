...
|----------|-----------|-------|------------|
| Throughput (low I/O) | 2.1x vs FPM | 0.9x vs FPM | 1.8x vs FPM |
| Throughput (high I/O) | 1.7x vs FPM | 3.2x vs FPM | 2.5x vs FPM |
...
- **RoadRunner**: Best default choice. No extension requirement, stable, excellent documentation, largest production deployments. Choose when uncertain.
- **Swoole**: Choose only when database/API latency is high (50ms+) and coroutine advantage is measurable in benchmarks.
- **FrankenPHP**: Choose when operational simplicity is paramount (single binary deployment, automatic HTTPS, no separate web server).
...
Octane Architecture | Performance Gain Estimation | Runtime Selection Decision Tree
