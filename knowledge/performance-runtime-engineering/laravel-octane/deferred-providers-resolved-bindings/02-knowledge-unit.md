...
**Defer default**: Make every custom service provider implement `DeferrableProvider` unless it registers event listeners, middleware, or route models in `boot()`. Only non-deferred providers should be those with boot-time side effects.

---

# Common Mistakes

**Pre-resolving everything**: Pre-resolving all services increases worker boot time proportionally. Worker start is already on the critical path for rolling deployments. Balance first-request speed against deployment speed.
...
Service Provider Optimization | State Management and Leak Prevention | Octane Service Container Lifecycle
