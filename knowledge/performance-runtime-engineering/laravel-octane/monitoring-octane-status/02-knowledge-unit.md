...
**Health check endpoint**: `/octane/health` returning `{ "status": "ok", "workers": 4, "active_requests": 2, "uptime_seconds": 3600 }`. Use in load balancer health checks. Worker crashes are detected when health check fails.

---

# Common Mistakes

**Not monitoring Octane workers in production**: A crashed worker silently reduces capacity. With 4 workers, one crash = 25% capacity loss. Always monitor worker count and alert on worker death.
...
Worker Configuration by Driver | State Management and Leak Prevention | FPM Status Page vs Octane Status
