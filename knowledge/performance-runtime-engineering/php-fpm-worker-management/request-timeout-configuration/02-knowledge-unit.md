...
Knowledge Unit: Request Timeout Configuration — request_terminate_timeout, max_execution_time, max_input_time
...
**Timeout hierarchy**: `max_input_time=60` > `max_execution_time=30` > `request_terminate_timeout=35`. Input time should be highest (wait for uploads), execution timeout catches runaway code, FPM timeout is the safety net.

---

# Common Mistakes

**Setting request_terminate_timeout too low**: If it fires during normal operation, users get 502 errors. Set at 2-3x p99 latency. Monitor request_terminate_timeout hits in FPM error log.
...
Slow Log Configuration and Analysis | FPM Status Page Monitoring | CPU vs I/O Bound Worker Ratios
