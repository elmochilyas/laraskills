...
- **CPU-bound bottleneck**: Heavy computation, image processing, encryption, complex algorithms. Optimization lever: JIT compilation, algorithmic improvements, opcode reduction.
- **I/O-bound bottleneck**: Database queries, HTTP API calls, file reads, network waits. Optimization lever: Coroutines (Swoole), persistent workers (Octane), connection pooling, caching.
- **Memory-bound bottleneck**: Large object graphs, data set processing, memory leaks. Optimization lever: GC tuning, memory_limit, OpCache sizing, pm.max_requests.
- **Framework overhead bottleneck**: Bootstrap cost dominating fast requests. Optimization lever: Octane/persistent workers, preloading, Composer optimization.

---

# Patterns

**Bottleneck diagnosis hierarchy**: 1) Measure p50 vs p95 latency gap (I/O variability), 2) Check CPU utilization during peak (CPU vs I/O bound), 3) Monitor RSS growth across requests (memory leak), 4) Profile a representative request.
...
Profiling vs Monitoring | Engine Version Performance Deltas | JIT Workload Benefit Assessment
