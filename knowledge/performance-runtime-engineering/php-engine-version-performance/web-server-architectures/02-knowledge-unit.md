...
Knowledge Unit: Web Server Architectures — CGI, FastCGI, PHP-FPM SAPI, Embedded SAPI
...
- **Embedded SAPI (SAPI_CGI or custom)**: PHP interpreter linked directly as a library. Used by FrankenPHP (CGO-embedded via Caddy), Apache mod_php (deprecated), and custom runtimes. Lowest latency, but requires thread-safe (ZTS) PHP compilation.
