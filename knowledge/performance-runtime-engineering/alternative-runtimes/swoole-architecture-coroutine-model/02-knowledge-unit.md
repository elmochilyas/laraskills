...
Knowledge Unit: Swoole Architecture and Coroutine Model — Event-Driven, One-Click Coroutineization
...
- **Coroutine**: Lightweight execution context (~2KB stack). Created per request. Yields on I/O (auto-hooked). Resume when I/O completes. No OS thread involvement.
...
- io_uring (Swoole 6.2+): Reduces syscall overhead for async I/O by using submission/completion queue pairs
...
Runtime Comparison Overview | Swoole Installation and Configuration | Swoole io_uring Integration
