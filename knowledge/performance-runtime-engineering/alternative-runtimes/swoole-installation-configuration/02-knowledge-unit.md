...
- **Installation**: `pecl install swoole` or compile from source with `--enable-swoole`. OpenSwoole is a community fork maintained separately.
- **worker_num**: Number of worker processes. Set to number of CPU cores for most workloads. `swoole_cpu_num()` returns the optimal value.
- **max_request**: Workers are recycled after handling this many requests. Prevents memory drift. Set 1000-5000 depending on application memory behavior.
...
- `worker_num` = CPU cores is optimal for CPU-bound; more for I/O-bound
- Swoole 6.2: io_uring support for async filesystem operations — significant improvement for file-heavy workloads

---

# Common Mistakes

**Setting worker_num too high (> CPU cores × 2)**: Context switching overhead degrades performance. Swoole's coroutine model handles concurrency within each worker — more workers just adds process scheduling overhead.

---

# Related Knowledge Units

Swoole Architecture and Coroutine Model | Swoole io_uring Integration | Laravel Octane Driver Selection
