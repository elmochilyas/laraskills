...
- **Caddy module**: FrankenPHP is a Caddy module written in Go. It handles TLS termination, HTTP/2/3 multiplexing, request routing, and static file serving — all within a single process.
...
- **Thread-safe PHP**: FrankenPHP relies on ZTS (Zend Thread Safety) compilation — each thread gets its own `TSRM` (Thread Safe Resource Management) context.
...
FrankenPHP Installation | FrankenPHP Worker Thread Management | FrankenPHP SAPI Internals
