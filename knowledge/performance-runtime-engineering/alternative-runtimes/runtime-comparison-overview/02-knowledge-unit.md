...
Knowledge Unit: Runtime Comparison Overview — Swoole, RoadRunner, FrankenPHP, ReactPHP, AMPHP
...
| Runtime | Architecture | Throughput vs FPM | Complexity | Extension Required |
|---------|-------------|-------------------|------------|-------------------|
| Swoole | Coroutine | 3-5x (I/O), 0.9x (no I/O) | High | Yes (C extension) |
| RoadRunner | Goroutine + PHP workers | 1.4-2.1x (all workloads) | Medium | No |
| FrankenPHP | Thread pool | 3-5x (worker mode) | Low | No (single binary) |
| ReactPHP | Event loop | 1.2-1.5x | Medium | No |

---

# Common Mistakes

**Choosing Swoole for low-I/O workloads**: With sub-1ms database queries, Swoole's coroutine overhead makes it slower than PHP-FPM. Benchmark with your actual workload before committing.
...
Architecture Model Differences | Runtime Selection Decision Tree | Laravel Octane Driver Selection
