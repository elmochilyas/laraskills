...
- **Cold-start latency**: Without preloading, first request after PHP-FPM restart compiles and autoloads framework classes ? 100-800ms overhead depending on application size.
- **Preloading cost**: Startup time increases by the time needed to compile and execute the preload script. For Laravel with 200 preloaded classes, ~200-500ms added to PHP-FPM startup.
- **Baseline memory**: Preloaded classes use memory equal to their compiled size + metadata. For a full Laravel preload (~800 classes), ~40-80MB additional OpCache consumption.
- **Benefit proportionality**: Fast APIs (<100ms) benefit most. Slow APIs (>500ms) see negligible improvement from preloading.

---

# Tradeoffs

| Aspect | Without Preloading | With Preloading |
|--------|-------------------|-----------------|
| First request latency | 200-800ms | 5-50ms |
| Steady-state latency | Same | Same |
| PHP-FPM startup time | 1-2s | 2-3s |
| OpCache memory usage | Low-Medium | Medium-High |

---

# Common Mistakes

**Preloading for slow applications**: If your application makes 200ms database queries, saving 16ms on autoloading yields <8% improvement. Preloading is a micro-optimization; fix I/O bottlenecks first.
...
Preloading Script Design Patterns | OpCache File Cache and Container Cold Start | OpCache Memory Sizing
