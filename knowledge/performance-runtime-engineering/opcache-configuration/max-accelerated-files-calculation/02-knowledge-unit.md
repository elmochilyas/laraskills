...
- **File counting**: `find /path -name "*.php" | wc -l` — count all PHP files in the project including vendor
- **Prime number rounding**: PHP's hash table performs best with prime-sized buckets. Values like 10000, 20000, 40000, 80000, 160000 are rounded to the nearest prime internally.
- **Default value**: 10000 (rounded to 10007) — sufficient for small apps but too low for Laravel/Symfony with vendor
- **Common values**: 20000 for medium apps, 40000 for large apps, 100000+ for monorepos with many packages

---

# Quick Reference

| Application | Typical File Count | Recommended max_accelerated_files |
|-------------|-------------------|-----------------------------------|
| WordPress (core + plugins) | 3,000-8,000 | 10,000 |
| Laravel (default) | 5,000-15,000 | 20,000 |
| Laravel + admin + packages | 15,000-30,000 | 40,000 |
| Symfony (full stack) | 8,000-20,000 | 20,000 |
| Monorepo / multi-app | 30,000-100,000+ | 100,000 |

---

# Common Mistakes

**Not counting vendor files**: `composer install` adds thousands of PHP files from dependencies. Always include vendor in the count.
...
OpCache Memory Sizing | OpCache Monitoring and Hit Rate | Production Hardening Settings
