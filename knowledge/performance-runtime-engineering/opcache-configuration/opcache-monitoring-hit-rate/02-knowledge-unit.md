...
- **cache_full flag**: Set to true when max_accelerated_files is exceeded. Requires OpCache reset to clear.
- **Wasted memory**: Internal fragmentation from file updates (when validate_timestamps=1) — old opcodes remain allocated but unreferenced. Reset OpCache periodically to reclaim.
...
**Ignoring cache_full**: Once max_accelerated_files is exceeded, new files are never cached but OpCache continues operating. The performance degradation is gradual and silent.
...
OpCache Memory Sizing | Max Accelerated Files Calculation | OpCache File Cache and Container Cold Start
